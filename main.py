...
import sys
import os
import json  # 改用 json 替代 eval 解析配置，更安全
...
def get_access_token():
    ...
    try:
        ...
        print("获取access_token失败，返回响应:", response)
        print("请检查app_id和app_secret是否正确")
        sys.exit(1)  # 已移除 os.system("pause")
    except Exception as e:
        print(f"获取access_token时发生网络或解析错误: {e}")
        sys.exit(1)
    return access_token
...
def send_message(to_user, access_token, region_name, weather, temp, wind_dir, note_ch, note_en):
    ...
    # 修正3：日期对象构造错误
    today = date(year, month, day)  # 原错误代码：datetime.date(datetime(year=year, month=month, day=day))
    ...
    if response["errcode"] == 40037:
        ...
        print("推送消息失败，错误响应:", response)


if __name__ == "__main__":
    # 使用 json 安全加载配置，替代 eval
    try:
        with open("config.txt", encoding="utf-8") as f:
            config = json.load(f)  # 改用 json.load
    except FileNotFoundError:
        print("推送消息失败，请检查config.txt文件是否与程序位于同一路径")
        sys.exit(1)
    except json.JSONDecodeError as e:
        print(f"推送消息失败，配置文件格式错误（不是有效的JSON）: {e}")
        print("请确保config.txt使用双引号，且格式正确")
        sys.exit(1)
    
    # 验证必需配置项
    required_configs = ["app_id", "app_secret", "user", "template_id", "region"]
    for req in required_configs:
        if req not in config:
            print(f"推送消息失败，配置文件中缺少必需的配置项: {req}")
            sys.exit(1)
